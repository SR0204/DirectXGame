...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	if (SUCCEEDED(device->QueryInterface(IID_PPV_ARGS(&infoQueue)))) {
		...
		D3D12_MESSAGE_ID denyIds[]{
			//Window11でのDXGIデバッグレイヤーとDX12デバッグレイヤーの相互作用バグによるメッセージ
	...
	rtvDescriptorHeapDesc.NumDescriptors = 2;//ダブルバッファように二つ。多くてもよい
	...
	//まず一つ目を作る。一つ目は最初のところに作る。作る場所もこちらが指定する。
	...
	inputElementDescs[0].SemanticName = "POSITOIN";
	...
	//Shaderをコンパイルする
	IDxcBlob* vertexShaderBlob = CompilerShader(L"Object3d.VS.hlsl", L"ps_6_0", dxcUtils, dxcCompiler, includeHandler);
	...
	while (msg.message != WM_QUIT) {
		...
		if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
			...
			//形状を設定.PSOに設定しているものとは別のもの。同じものを設定すると考えておけばいい
			commandList->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
			//描画(DrawCall).３頂点で一つのインスタンス。
			commandList->DrawInstanced(3, 1, 0, 0);


			//画面に描く処理はすべて終わり。画面に映すので状態を還移
