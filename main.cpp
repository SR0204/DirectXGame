...
{
	//matedateを基にResourceも設定
	D3D12_RESOURCE_DESC resourceDesc{};
	resourceDesc.Width = UINT(metadate.width);//Textureの幅
	resourceDesc.Height = UINT(metadate.height);//Textureの高さ
	resourceDesc.MipLevels = UINT16(metadate.mipLevels);//mipmapの数
	resourceDesc.DepthOrArraySize = UINT16(metadate.arraySize);//奥行きor配列Textureの配列数
	resourceDesc.Format = metadate.format;//TextureのFormat
	resourceDesc.SampleDesc.Count = 1;//サンプリングカウント
	resourceDesc.Dimension = D3D12_RESOURCE_DIMENSION(metadate.dimension);//Textureの次元数

	//利用するHeapの設定
	D3D12_HEAP_PROPERTIES heapProperties{};
	heapProperties.Type = D3D12_HEAP_TYPE_CUSTOM;//細かい設定を行う
	heapProperties.CPUPageProperty = D3D12_CPU_PAGE_PROPERTY_WRITE_BACK;//WriteBackポリシーでCPUアクセス可能
	heapProperties.MemoryPoolPreference = D3D12_MEMORY_POOL_L0;//プロセッサの近く設置
...
ID3D12Resource* UploadTextureDate(ID3D12Resource* texture, const DirectX::ScratchImage& mipImages,ID3D12Device*device,ID3D12GraphicsCommandList* CommandList)
{
	...
	std::vector<D3D12_SUBRESOURCE_DATA> subresouroces;
	DirectX::PrepareUpload(device, mipImages.GetImages(), mipImages.GetImageCount(), mipImages.GetMetadata(), subresouroces);
	uint64_t intermediateSize = GetRequiredIntermediateSize(texture, 0, UINT(subresouroces.size()));
	ID3D12Resource* intermediateResource = CreateBufferResource(device, intermediateSize);
	UpdateSubresources(CommandList, texture, intermediateResource, 0, 0, UINT(subresouroces.size()), subresouroces.data());
	//Tetureへの転送後は利用できるように、D3D12_RESOURCE_STATE_COPY_DESTからD3D12_RESOURCE_STATE_GENERIC_READへResourceStateを変更する
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	*wvpData = MakeIdentity4x4();
	...
	rootParameters[2].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;//DescriptorRableを使う
	...
	staticSamplers[0].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;//PixelSharderで使う
